...
const getFriendsByUserId = async (req, res) => {
    try {
        ...
        if (userId === req.currentUserId || (await isFriends(req.currentUserId, userId))) {
            ...
            const formatted = friendships.map((friendship) => (friendship.user_id.id === userId ? friendship.friend_id : friendship.user_id));
...
const acceptFriendsRequest = async (req, res) => {
    try {
        ...
        const friendship = await Friendship.findOneAndUpdate({ user_id: req.params.fid, friend_id: req.params.id, status: "pending" }, { status: "accepted" }, { new: true });
